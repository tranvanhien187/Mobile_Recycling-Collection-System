...
import datn.cnpm.rcsystem.databinding.FragmentCompleteFormBinding
import java.io.File

@AndroidEntryPoint
class CompleteFormFragment : BaseFragment<FragmentCompleteFormBinding>() {

    override val bindingInflater: (LayoutInflater, ViewGroup?, Boolean) -> FragmentCompleteFormBinding
        get() = FragmentCompleteFormBinding::inflate

    val viewModel: CompleteFormViewModel by viewModels()
    ...
    override fun initObservers() {
        ...
        viewModel.liveEvent.observe(viewLifecycleOwner) { event ->
            when (event) {
                is CompleteFormEvent.EvidenceEmpty -> {
                    showError("Evidence is not empty.")
                }
                is CompleteFormEvent.CompleteTransportGarbageSuccess -> {
                    showDialogConfirm(
